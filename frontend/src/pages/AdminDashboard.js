...
import {
  ...
  FaSearch, FaUsers, FaFlag, FaEye, FaTimes
} from "react-icons/fa";
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [usr, tsk, srv, rep] = await Promise.all([
  ...
  const filteredItems = (
    ...
    const searchVal =
      ...
      activeTab === "reports" ? item.reporter_name :
      item.title;
  ...
  return (
    <div className="db-page">
            ...
            <thead className="bg-[#f8f9fc]">
              <tr>
                <th className="px-8 py-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">
                  {activeTab === "users"   ? "User Identity"   :
                   activeTab === "reports" ? "Reporter"        : "General Information"}
                </th>
                <th className="px-8 py-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">
                  {activeTab === "services" ? "Provider"       :
                   activeTab === "tasks"    ? "Client"         :
                   activeTab === "reports"  ? "Entity At Fault" : "Privilege"}
                </th>
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-5">
                    <div className="font-bold text-slate-900 text-sm">
                      {activeTab === "users"   ? item.name :
                       activeTab === "reports" ? item.reporter_name : item.title}
                    </div>
                    <div className="text-xs text-slate-400 mt-1">
                      {activeTab === "users"   ? item.email :
                       activeTab === "reports" ? <span className="text-red-500 font-semibold">{item.reason}</span> :
                  ...
                  <td className="px-8 py-5">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight ${
                      ...
                      {activeTab === "services" ? item.provider_name :
                       ...
                       activeTab === "reports"  ? `${item.target_type} #${item.target_id || item.id}` :
                       item.role}
                    </span>
                  </td>
      ...
      {selectedReport && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-[#0a0a0a]/60 backdrop-blur-md p-4"
        ...
        >
          ...
          >
            <div className="px-8 py-6 bg-[#f8f9fc] border-b border-slate-100 flex justify-between items-center">
              ...
              <button
                onClick={() => setSelectedReport(null)}
                className="bg-white w-8 h-8 rounded-full shadow-sm flex items-center justify-center text-slate-400 hover:text-black transition-colors"
              >
                <FaTimes />
              </button>
            </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-[#0a0a0a] capitalize">
                      {selectedReport.target_type} ID: {selectedReport.target_id || selectedReport.id}
                    </p>
                  </div>
                </div>
